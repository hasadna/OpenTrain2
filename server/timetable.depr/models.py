...
class TtTrip(models.Model):
    ...
    def print_stoptimes(self):
        ...
        for stop in stop_times:
            arrival_str = ot_utils.get_localtime(stop.exp_arrival).strftime('%H:%M:%S') if ot_utils.get_localtime(
                stop.exp_arrival) is not None else '--:--:--'
            departure_str = ot_utils.get_localtime(stop.exp_departure).strftime('%H:%M:%S') if ot_utils.get_localtime(
                stop.exp_departure) is not None else '--:--:--'
