...
def get_expected_location(trip, dt):
    ...
    if not before_stop:
        return (
        [after_stop.stop.stop_lat, after_stop.stop.stop_lon], [after_stop.stop.stop_lat, after_stop.stop.stop_lon])
    if not after_stop or after_stop == before_stop:
        return (
        [before_stop.stop.stop_lat, before_stop.stop.stop_lon], [before_stop.stop.stop_lat, before_stop.stop.stop_lon])
...
def find_min_dist_to(shape, stop):
    ...
    dists = [(common.ot_utils.latlon_to_meters(point[0], point[1], latlon[0], latlon[1]), idx) for idx, point in
             enumerate(points)]
    result = min(dists)
...
def create_dists_csv():
    ...
    with open(os.path.join(settings.BASE_DIR, 'timetable/static/timetable/dists.csv'), 'w') as fh:
        ...
        writer.writerow(
            ['stop_id1', 'stop_name1', 'stop_id2', 'stop_name2', 'num_results', 'distance1', 'aerial_distance1',
             'distance2', 'aerial_distance2'])
        for entry in matrix:
